...
index a5c02dda57c7ac3bb7b08d56f47581026ed6bc29..5152f0a26cc8ce35ae636435511903fece3f38f7 100644
...
+        val pinHash = prefs.getString(KEY_ADMIN_PIN_HASH, null)
...
+        askPin("Digite o PIN atual") currentPin@{ current ->
...
+            askPin("Digite o novo PIN") newAdminPin@{ newPin ->
                 if (newPin.length < 4) {
                     toast("PIN deve ter ao menos 4 dígitos")
-                    return@askPin
+                    return@newAdminPin
...
+        }
+
+        askPin("Digite o PIN Usuário atual") currentUserPin@{ current ->
+            if (hash(current) != currentHash) {
+                toast("PIN atual inválido")
+                return@currentUserPin
+            }
+
+            askPin("Digite o novo PIN Usuário") newUserPin@{ newPin ->
+                if (newPin.length < 4) {
+                    toast("PIN deve ter ao menos 4 dígitos")
+                    return@newUserPin
+                }
...
+                toast("PIN Usuário removido")
     ...
     private fun addWhitelistApp() {
         ...
         AlertDialog.Builder(this)
             ...
             .show()
@@ -225,50 +286,51 @@ class MainActivity : AppCompatActivity() {
             }
...
+        pendingUserAuthAfterWhitelistedApp = true
     ...
     private fun launchApp(packageName: String) {
         ...
         // Fora de Device Owner, abrir outro app exige sair do lock task atual.
         // Ao retornar para o kiosk, o lock task é reativado no onResume.
         relockOnResume = true
...
@@ -276,67 +338,99 @@ class MainActivity : AppCompatActivity() {
...
+        val pinHash = prefs.getString(KEY_ADMIN_PIN_HASH, null)
         if (pinHash.isNullOrBlank()) {
-            toast("Defina um PIN antes de sair do modo kiosk")
...
@@ -345,35 +439,32 @@ class MainActivity : AppCompatActivity() {
