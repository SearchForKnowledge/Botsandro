    ...
    private var lastExternalPackage: String? = null
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            refreshWhitelistUi()
            // ✅ Device Owner: aplica HOME e whitelist
    applyCorporateHomeAsDefault()

    // ✅ Entra em locktask se permitido
...
setOnClickListener {
    ...
    // Define seu app como HOME padrão (launcher corporativo)
    private fun applyCorporateHomeAsDefault() {
    ...
    // Remove o HOME padrão
    private fun clearCorporateHome() {
        if (!isDeviceOwner()) return
        try {
    ...
    // Aplica whitelist do LockTask (kiosk + apps liberados)
    private fun applyLockTaskWhitelistFromPrefs() {
        if (!isDeviceOwner()) return

        val whitelist = prefs.getStringSet(KEY_WHITELIST, emptySet()) ?: emptySet()

        // Sempre permitir o próprio app também
